...
( function( blocks, element, serverSideRender, i18n, blockEditor, components ) {
	...
	blocks.registerBlockType(
		'wp-events/event-end',
		{
			title: __( 'Event End Date/Time', 'wp-events' ),
			icon: 'clock',
			category: 'wp-events',
			...
			edit: function( props ) {
				...
				el(
					...
					el(
						PanelBody,
						{ title: __( 'Date/Time Settings', 'wp-events' ) },
						el(
							ToggleControl,
							{
						...
						attributes.dateFormat === 'custom' && el(
							TextControl,
							{
						...
						el(
							ToggleControl,
							{
								label: __( 'Hide date if same day as start', 'wp-events' ),
								checked: attributes.hideIfSameDay,
								onChange: function( value ) {
									setAttributes( { hideIfSameDay: value } );
								}
							}
						)
					)
