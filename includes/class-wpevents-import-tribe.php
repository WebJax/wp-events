...
class WPEvents_Import_Tribe {
	public static function register() {
		...
		add_action( 'admin_post_wpevents_import_tribe_selected', array( __CLASS__, 'handle_import_selected' ) );
	...
	public static function render_tools_page() {
		...
		if ( isset( $_GET['msg'] ) ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html( sanitize_text_field( wp_unslash( $_GET['msg'] ) ) ) . '</p></div>';
		}
	...
	protected static function get_tribe_events( $filter = 'all' ) {
		...
		foreach ( $query->posts as $post ) {
			...
			if ( 'imported' === $filter && ! $imported_id ) {
				continue;
			}
			if ( 'available' === $filter && $imported_id ) {
				continue;
			}
		...
		if ( ! isset( $_POST['event_ids'] ) || ! is_array( $_POST['event_ids'] ) ) {
			wp_safe_redirect(
				add_query_arg(
					array(
						'page' => 'wpevents-import-tribe',
						'msg'  => rawurlencode( 'No events selected.' ),
					),
					admin_url( 'edit.php?post_type=event' )
				)
			);
	...
	public static function run_import( $batch = 0, $filter = 'available' ) {
		...
		foreach ( $q->posts as $tribe_id ) {
			...
			if ( $new_id ) {
				$imported++;
				update_post_meta( $tribe_id, '_wpevents_imported', $new_id ); } else {
				$skipped++; }
		}
		return array(
			'imported' => $imported,
	...
	protected static function import_single( $tribe_id ) {
		...
		if ( $currency ) {
			update_post_meta( $event_id, 'event_currency', strtoupper( $currency ) );
		}

		// Map venue.
		$venue_id = self::map_or_create_venue( $tribe_id );
		if ( $venue_id ) {
			update_post_meta( $event_id, 'event_venue', $venue_id );
		}

		// Map organizers (can be multiple)
		$org_ids = self::map_or_create_organizers( $tribe_id );
		if ( $org_ids ) {
			update_post_meta( $event_id, 'event_organizer', $org_ids );
		}

		// Import categories from tribe_events_cat to event_category.
		$tribe_cats = wp_get_object_terms( $tribe_id, 'tribe_events_cat', array( 'fields' => 'names' ) );
		...
		$tribe_tags = wp_get_object_terms( $tribe_id, 'post_tag', array( 'fields' => 'names' ) );
	...
	protected static function map_or_create_organizers( $tribe_event_id ) {
		...
		if ( is_array( $multi ) ) {
			$orgs = array_merge( $orgs, array_map( 'absint', $multi ) ); }
