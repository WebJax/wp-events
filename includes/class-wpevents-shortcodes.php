...
class WPEvents_Shortcodes {
	...
	public static function events_list( $atts ) {
		...
		if ( ! $q->have_posts() ) {
			return '<div class="wp-events-list empty">' . esc_html__( 'No upcoming events', 'wp-events' ) . '</div>';
		}
	...
	public static function event_single( $atts ) {
		$atts    = shortcode_atts( array( 'id' => 0 ), $atts, 'event' );
		...
		if ( $start ) {
			...
			echo '<time datetime="' . esc_attr( $start ) . '">' . esc_html( wp_date( 'j. M Y H:i', strtotime( $start ) ) ) . '</time>';
			if ( $end ) {
				echo ' - <time datetime="' . esc_attr( $end ) . '">' . esc_html( wp_date( 'j. M Y H:i', strtotime( $end ) ) ) . '</time>';
			}
			echo '</div>';
		}
		if ( $venue_id ) {
		...
		if ( ! empty( $organizers ) ) {
			$names = array_map(
				function( $oid ) {
					return get_the_title( (int) $oid );
				},
				$organizers
			);
			$names = array_filter( $names );
			if ( $names ) {
				echo '<div class="organizers"><strong>' . esc_html__( 'Organizer(s)', 'wp-events' ) . ':</strong> ' . esc_html( implode( ', ', $names ) ) . '</div>';
			}
		}
		echo '</div>';
		return ob_get_clean();
