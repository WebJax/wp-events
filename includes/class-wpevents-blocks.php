...
class WPEvents_Blocks {
	public static function register() {
		...
		add_action( 'init', array( __CLASS__, 'register_scripts' ), 9 ); // Register scripts first
		add_action( 'init', array( __CLASS__, 'register_blocks' ), 10 ); // Then register blocks
	...
	public static function register_blocks() {
		...
		register_block_type(
			'wp-events/event-venue',
			array(
				'render_callback' => array( __CLASS__, 'render_event_venue' ),
				'category'        => 'wp-events',
		...
		register_block_type(
			...
			array(
				'render_callback' => array( __CLASS__, 'render_event_organizer' ),
				'category'        => 'wp-events',
		...
		register_block_type(
			'wp-events/event-start',
			array(
				'render_callback' => array( __CLASS__, 'render_event_start' ),
				'category'        => 'wp-events',
		...
		register_block_type(
			'wp-events/event-end',
			array(
				'render_callback' => array( __CLASS__, 'render_event_end' ),
				'category'        => 'wp-events',
		...
		if ( file_exists( __DIR__ . '/../blocks/events-list.js' ) ) {
			register_block_type(
				...
				array(
					'render_callback' => array( __CLASS__, 'render_events_list' ),
					'category'        => 'wp-events',
		...
		if ( file_exists( __DIR__ . '/../blocks/events-carousel.js' ) ) {
			register_block_type(
				...
				array(
					'render_callback' => array( __CLASS__, 'render_events_carousel' ),
					'category'        => 'wp-events',
	...
	public static function register_scripts() {
		...
		$deps = array( 'wp-blocks', 'wp-element', 'wp-server-side-render', 'wp-i18n', 'wp-block-editor', 'wp-components' );
		...
		wp_register_style( 'wp-events-blocks', plugins_url( '../assets/wp-events.css', __FILE__ ), array(), WPEVENTS_VERSION );
	}
	...
	public static function render_events_list( $attributes = array(), $content = '' ) {
		// Reuse shortcode output
		return WPEvents_Shortcodes::events_list( array( 'limit' => isset( $attributes['limit'] ) ? $attributes['limit'] : 5 ) );
	}

	public static function render_events_carousel( $attributes = array(), $content = '' ) {
		$atts = array( 'limit' => isset( $attributes['limit'] ) ? $attributes['limit'] : 5 );
		$q    = new WP_Query(
			array(
				'post_type'      => 'event',
		...
		if ( ! $q->have_posts() ) {
			return '<div class="wp-events-carousel empty">' . esc_html__( 'No upcoming events', 'wp-events' ) . '</div>';
		}
		ob_start();
	...
	public static function render_event_venue( $attributes = array(), $content = '', $block = null ) {
		$post_id = get_the_ID();
		if ( ! $post_id ) {
			global $post;
	...
	public static function render_event_organizer( $attributes = array(), $content = '', $block = null ) {
		$post_id = get_the_ID();
		if ( ! $post_id ) {
			global $post;
			$post_id = $post ? $post->ID : 0;
	...
	public static function render_event_start( $attributes = array(), $content = '', $block = null ) {
		$post_id = get_the_ID();
		if ( ! $post_id ) {
			global $post;
			$post_id = $post ? $post->ID : 0;
	...
	public static function render_event_end( $attributes = array(), $content = '', $block = null ) {
		$post_id = get_the_ID();
		if ( ! $post_id ) {
			global $post;
			$post_id = $post ? $post->ID : 0;
