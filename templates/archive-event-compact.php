<?php
/**
 * Template for displaying Events archive page - Compact View
 * 
 * This file can be overridden by copying it to yourtheme/wp-events/archive-event-compact.php
 * Shows events in a compact list format with minimal information
 * 
 * @package WP Events
 * @version 1.0.0
 */
...
<div class="wp-events-archive-page wp-events-compact-view">
    <div class="container">
        <header class="archive-header">
            <h1 class="archive-title">
                <?php 
                if ( is_post_type_archive( 'event' ) ) {
                    echo esc_html__( 'Kommende Events', 'wp-events' );
                } else {
        ...
        <div class="wp-events-archive">
            <?php if ( have_posts() ) : ?>
                <div class="events-compact-container">
                    <?php while ( have_posts() ) : the_post(); 
                        // Load compact card template
                        get_template_part( 'wp-events/parts/event-card-compact' );
                        if ( ! locate_template( 'wp-events/parts/event-card-compact.php' ) ) {
                            include WPEVENTS_PLUGIN_DIR . 'templates/parts/event-card-compact.php';
                        }
                    endwhile; ?>
                </div>
