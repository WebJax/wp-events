<?php
/**
 * Template for displaying Events archive page - Calendar/Grid View
 * 
 * This file can be overridden by copying it to yourtheme/wp-events/archive-event-calendar.php
 * Shows events in a multi-column grid layout (like a calendar view)
 * 
 * @package WP Events
 * @version 1.0.0
 */
...
<div class="wp-events-archive-page wp-events-calendar-view">
    <div class="container">
        <header class="archive-header">
            <h1 class="archive-title">
                <?php 
                if ( is_post_type_archive( 'event' ) ) {
                    echo esc_html__( 'Event Kalender', 'wp-events' );
                } else {
        ...
        <div class="wp-events-archive">
            <?php if ( have_posts() ) : ?>
                <div class="events-grid events-grid-3col">
                    <?php while ( have_posts() ) : the_post(); 
                        // Load grid card template
                        get_template_part( 'wp-events/parts/event-card-grid' );
                        if ( ! locate_template( 'wp-events/parts/event-card-grid.php' ) ) {
                            include WPEVENTS_PLUGIN_DIR . 'templates/parts/event-card-grid.php';
                        }
                    endwhile; ?>
                </div>
